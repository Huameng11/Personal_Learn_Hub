...
### 4. 独立Docker Compose安装（建议采用下方最新版docker安装）
...
一次性把 Docker Engine、containerd 以及 Docker Compose 插件**全部更新到最新稳定版**
...
**如需修改节点,v2ray导出后，对应修改inbounds中关于端口的listen和其他ip即可，字段大体如下，会有不同的端口**
...
- 当前部署：Docker Compose 标准部署，数据库已使用官方 VectorChord 镜像，无需迁移向量引擎

- 版本控制：`${IMMICH_VERSION:-release}` 会自动拉取最新稳定版，无需手动写死版本号

- 更新前必做：数据库备份，防止升级异常丢失数据
...
```shell
# 进入compose.yml所在目录执行
docker compose exec -it immich_postgres pg_dump -U ${DB_USERNAME} ${DB_DATABASE_NAME} > immich_backup_$(date +%Y%m%d).sql
```

备份文件保存到本地，升级出问题可恢复。

3. **核对 docker-compose.yml 差异并优化**

database 配置完全符合 VectorChord 规范，无需增删参数，建议修改如下：

- 官方 vectorchord 专用镜像哈希值
- 配置`shm_size: 128mb`

**4. Immich 完整更新流程**

- 步骤 1：修改并保存优化后的 docker-compose.yml

- 步骤 2：在 `immich/docker` 目录下运行拉取所有服务最新镜像

```shell
docker compose pull
```

自动拉取 immich-server、ml、postgres、valkey 最新镜像
...
- 首次启动会自动执行数据库迁移、重建人脸 / 图片向量索引
- 图片资产十万级以上会启动缓慢，无报错日志耐心等待，不要强制重启

- 步骤 4：清理旧镜像释放磁盘空间（务必确保容器正常启动后再执行，防止删除正常镜像）
...
`Error response from daemon: driver failed programming external connectivity on endpoint immich_server (e80f630d6adaa4aac1a4342b97c3da8f348c05bb15c05ef30cb83898edba8507): Bind for 0.0.0.0:2283 failed: port is already allocated`

`immich-server`容器启动时可能会出现端口占用情况，究其原因可能是更新大版本，旧容器占用导致。处理如下

1. 步骤 1：彻底停止所有容器 + 清理网络
...
按提示输入两次密码（**可以和系统登录密码不同，建议设置一下**）。华梦那套密码

设置密码后无法再次查看明文密码，只能再次使用该命令修改密码。
...
docker compose down
cp -r config config_backup_20260728  # 复制配置目录作为备份
```

**2.拉取最新的稳定版镜像**

在 `docker-compose.yml` 所在的目录下，执行命令让 Docker 去官方仓库拉取最新的稳定版（`stable`）镜像：
...
拉取完成后，直接一键启动。Docker Compose 会自动发现有新镜像，并安全地销毁旧容器、创建基于新版本镜像的新容器：
...
你可以通过以下命令查看容器日志，确认 HA 是否正常启动且没有报错：

```bash
docker compose logs -f homeassistant
```

稍等一两分钟后，刷新HA 网页端（http://树莓派IP:8123），进入 **设置 -> 系统 -> 关于**。
...
树莓派Docker日志占用磁盘爆满排查与解决教程

## 九、磁盘莫名占满问题排查与解决步骤

本次故障现象：树莓派SSD系统盘无手动存放大量文件，但磁盘使用率极高，磁盘被莫名占满。最终定位根本原因为：**Docker容器运行日志无限累积暴涨，占用数百G磁盘空间**，属于Docker最常见的爆盘故障。下面为标准化排查、清理、永久根治完整流程。

### 1、排查磁盘整体占用情况

首先查看系统全盘磁盘使用率，确认磁盘占用状态。

```bash
df -h
```

现象特征：根分区 `/dev/sda2`使用率极高，磁盘即将占满。

### 2、定位系统占用最大目录

遍历根目录所有文件夹，排序找出占用空间最大的目录，锁定故障目录。

```bash
sudo du -sh /* 2>/dev/null | sort -rh | head -10
```

本次排查结果：**/var 目录占用292G**，为磁盘占用元凶。

### 3、逐层精准定位/var下占用目录

进入/var目录深度排查，找到占用空间最大的子目录。

```bash
sudo du -sh /var/* 2>/dev/null | sort -rh | head -10
```

排查结果：**/var/lib 占用292G**，进一步锁定Docker目录。

### 4、确认Docker目录占用大小

单独查询Docker根目录磁盘占用，确认故障来源。

```bash
sudo du -sh /var/lib/docker
```

排查结果：**/var/lib/docker 占用291G**，确认磁盘空间全部被Docker占用。

### 5、区分Docker真实业务与垃圾占用

查询Docker官方统计信息，区分是镜像、容器、数据卷还是日志垃圾占用，避免误删业务数据。

```bash
sudo docker system df
```

关键结果解读：Docker镜像、容器、数据卷仅占用不足8G，说明**并非业务数据占用，是隐藏日志文件暴涨**。

### 6、精准定位Docker内部超大目录

深度遍历Docker子目录，找到真正爆盘的文件夹。

```bash
sudo find /var/lib/docker -maxdepth 1 -type d -exec du -sh {} \; | sort -rh
```

最终定位元凶：**/var/lib/docker/containers 占用277G**，该目录存放所有容器运行日志。

### 7、安全清理Docker暴涨日志（不影响业务）

该目录下的 `*-json.log` 为容器运行日志，属于纯垃圾文件，清空后**不删除容器、不丢失业务数据、不影响服务运行**。
...
清理完成后，再次查看磁盘使用率，确认空间释放成功。

```bash
df -h
```

预期效果：根分区磁盘使用率大幅下降，释放270G+存储空间。

### 9、永久根治：限制Docker日志大小（核心步骤）

Docker默认无日志大小限制，会无限累积占用磁盘。通过配置文件限制日志最大容量，彻底杜绝后续爆盘问题。

1. 创建Docker全局配置文件，写入日志限制规则：单个日志文件最大10M，最多保留3个日志文件
